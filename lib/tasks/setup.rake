namespace :setup do
  ...
  task(omop_tables: :environment) do |t, args|
    ...
    [Person, Provider, Observation, Measurement].each do |klass|
      omop_table        = OmopTable.new
      omop_table.name   = klass.table_name
      omop_table.domain = klass.try(:domain)
      omop_table.save!
      ...
      instance.attributes.keys.each do |attribute|
        ...
        omop_column.data_type = instance.column_for_attribute(attribute).type
        omop_column.map_type  = klass.map_types[attribute.to_sym]
        omop_column.save!
      end
    end
  end
  ...
  namespace :neurofiles do
    ...
    task projects: :environment do |t, args|
      redcap_project = RedcapProject.where(project_id: 5840).first_or_initialize
      ...
      redcap_project = RedcapProject.where(project_id: 5843).first_or_initialize
      ...
      redcap_project = RedcapProject.where(project_id: 5844).first_or_initialize
    ...
    task project_sandbox: :environment do |t, args|
      redcap_project = RedcapProject.where(project_id: 5912).first_or_initialize
